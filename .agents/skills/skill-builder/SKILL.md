---
name: skill-builder
description: Use when creating new skills, optimizing existing skills, or auditing skill quality. Guides skill development following Codex official best practices.
---

## What This Skill Does

Guides the creation and optimization of Codex skills using official best practices. Use this whenever:
...
A skill is a reusable set of instructions that tells Codex how to handle a specific task. Skills live in `.Codex/skills/[skill-name]/SKILL.md` inside your project. When you type `/skill-name` or describe what you need in natural language, Codex loads the skill's instructions and follows them.
...
- Your project's `AGENTS.md` instructions are always loaded, every conversation
- Skill *descriptions* (from frontmatter) are always loaded so Codex knows what's available
- The full skill content only loads when the skill is actually invoked
- Once loaded, Codex follows the skill's instructions while still respecting your AGENTS.md rules
...
**Round 2: Trigger**
*Why this matters: The `description` field is how Codex decides whether to load your skill. Bad trigger words mean Codex never uses it. Too broad means Codex fires it when you don't want it.*

- What would someone say to trigger this? (Get 2-3 natural language phrases)
- Should it be user-only (`/slash-command`), Codex-auto-invocable, or both?
- Does it accept arguments? If so, what? (e.g., a topic, a URL, a file path)

**Round 3: Step-by-Step Process**
*Why this matters: Codex follows instructions literally. Vague steps produce vague results. Specific steps produce consistent output every time.*

- Walk me through exactly what should happen from trigger to output. What's step 1? Step 2? Keep going.
- For each step: Does Codex do it directly, or delegate to a subagent/script?
...
- **Reference skills** add knowledge Codex applies to current work without performing an action. Examples: coding conventions, API patterns, style guides.
...
- `disable-model-invocation: true` -- Set if the skill has side effects (file generation, API calls, costs money). Prevents Codex from auto-invoking.
...
2. **Step-by-step workflow** -- Numbered steps. Each step tells Codex exactly what to do.
...
If your skill needs detailed reference docs, examples, or scripts, add them alongside SKILL.md in the same directory. Reference them from SKILL.md so Codex knows they exist. Supporting files are NOT loaded automatically -- they load only when Codex needs them. See [reference.md](reference.md) for the full pattern.

**Step 5: Document in AGENTS.md**

Your project's `AGENTS.md` file is where Codex loads project-wide instructions every conversation. After creating a skill, add a brief entry so you (and your team) know what's available:
...
This isn't required for the skill to work, but it keeps your project organized and helps Codex understand how skills fit into your broader workflow.
...
**File:** `.Codex/skills/meeting-notes/SKILL.md`
...
- [ ] No vague instructions -- every step tells Codex exactly what to do
- [ ] String substitutions (`$ARGUMENTS`, `$N`) are used where the skill takes input

### Integration Audit

- [ ] Skill is documented in AGENTS.md (recommended, not required)
...
- [ ] Doesn't duplicate information that lives elsewhere (AGENTS.md, other skills)
...
- Document all active skills in your project's AGENTS.md
